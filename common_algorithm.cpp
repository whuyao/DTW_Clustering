...
#pragma omp parallel for num_threads(max_num_threads<num?max_num_threads:num) schedule(dynamic)
		for (i = 0; i < num; i++)  
		{  
			/////////////////////////////////	
			double total_val = 0;		
...
bool outputFile( vector<Tuple>* clusters, int nClusterNum, char* outputfilename )
{
	...
	_silin<<"nIteration\t\tsilhouette_value"<<endl;
...
// 			cout<<"processing No. "<<i+1<<" / "<<nDataCount<<"..."<<endl;
		...
		for (j=0; j<nDataCount; j++)
		{
			...
			pDist[nOtherClusterClassID] += i >= j ? ppDistance[i][j]:ppDistance[j][i];
		}
		...
		for (j=0; j<nClusterNum; j++)
		{
			...
			pDist[j] /= nClusterSize;			 
		}

		//与自己类别的距离
		double _a = pDist[nClassID];
		double _b = 10e6;
		for (j=0; j<nClusterNum; j++)
		{
			if (j == nClassID)
